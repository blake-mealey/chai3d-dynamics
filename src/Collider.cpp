...
Collider::Collider(const double a_collisionStiffness, const double a_dampingConstant, const size_t a_collisionFlag, const size_t a_collidesWithMask) :
...
void Collider::AddCollisionForces(Collider *a_collider, bool& a_result0, bool& a_result1) {
    ...
    const bool body0CollidesWithBody1 = (m_collidesWithMask & a_collider->m_collisionFlag) != 0;
    const bool body1CollidesWithBody0 = (a_collider->m_collidesWithMask & m_collisionFlag) != 0;
    if (!body0CollidesWithBody1 && !body1CollidesWithBody0) return;

    // Get the collision normal and return if there was no collision
    const Collision collision = GetCollision(a_collider);
    if (!collision.valid) return;

    // Get stiffness and constants
    const double collisionStiffness0 = m_collisionStiffness;
    const double dampingConstant0 = m_dampingConstant;
    const double collisionStiffness1 = a_collider->m_collisionStiffness;
    const double dampingConstant1 = a_collider->m_dampingConstant;
    ...
    const cVector3d relativeVelocity = body1->GetLinearVelocity() - body0->GetLinearVelocity();
    ...
    if (body0CollidesWithBody1 && m_currentCollisionCount < Constants::MAX_COLLISIONS) {
        ...
        const cVector3d collisionForce0 = -collisionStiffness0 * collision.normal;
        const cVector3d collisionDampingForce0 = -dampingConstant0 * cProject(relativeVelocity, collision.normal);
        const cVector3d netForce0 = collisionForce0 + collisionDampingForce0;
    ...
    if (body1CollidesWithBody0 && a_collider->m_currentCollisionCount < Constants::MAX_COLLISIONS) {
        ...
        // Calculate and add force of collision (with damping)
        const cVector3d collisionForce1 = -collisionStiffness1 * collision.normal;
        const cVector3d collisionDampingForce1 = -dampingConstant1 * cProject(relativeVelocity, collision.normal);
        const cVector3d netForce1 = collisionForce1 + collisionDampingForce1;
...
void Collider::SetCollisionFlag(const size_t a_collisionFlag) {
...
void Collider::SetCollidesWithMask(const size_t a_collidesWithMask) {
...
void Collider::SetCollisionStiffness(const double a_collisionStiffness) {
...
void Collider::SetDampingConstant(const double a_dampingConstant) {
